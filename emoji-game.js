...
const winFinal_audio = $('#win-final');
const flip_audio = $('#flip');
const oneWin_audio = $('#one-win');
const clickable_audio = $('#clickable');
...
function resume() {
  ...
  baseEmojis = LEVELS[levelIndex].emojis;
  start();
}
...
$('#new-game').addEventListener('click', () => {
  function newGame() {
    isRandom = true;
    const PLI = parseInt(Math.random() * LEVELS.length);
    baseEmojis = LEVELS[PLI].emojis;
    playingLevel = PLI;
    start();
  }
  newGame();
...
function timer() {
  second =
    LEVELS[playingLevel ? parseInt(playingLevel) : 0].time * 60;
...
function incrementLevel() {
  ...
  $('#level-img').src = `https://flip.chattgo.com/assets/emojis/${
    LEVELS[level] ? LEVELS[parseInt(level)].emojis[0].emoji : 'logo'
  }.svg`;
  baseEmojis = LEVELS[level];
}
...
function play() {
  ...
  renderEmojis.forEach((emoji, i) => {
    ...
    const text = document.createTextNode(
      LEVELS[playingLevel ? parseInt(playingLevel) : 0].name
    );
    ...
    div.addEventListener('click', (e) => {
      if (!wins.includes(parseInt(i))) {
        if (flippedIndex.length <= 1) {
          ...
          clickable_audio.pause();
          clickable_audio.currentTime = 0;
          clickable_audio.play();
